...
async fn doc_move_rejects_missing_target_folder() {
    ...
    let args = syo_cli::commands::doc::MoveArgs {
    ...
    let cmd = syo_cli::commands::doc::DocCmd::Move(args);
    let result = syo_cli::commands::doc::run(&f.client, cmd).await;
...
async fn create_doc_rejects_duplicate_hpath() {
    ...
    let args = syo_cli::commands::doc::CreateDocArgs {
        notebook: f.notebook_id.to_string(),
        hpath: "/ConflictTest".to_string(),
        markdown_file: "-".to_string(),
        force: false,
    };
    let result = syo_cli::commands::doc::create::run(&f.client, args).await;
